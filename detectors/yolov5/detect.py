# YOLO detection wrapper - Full implementation
import cv2
import numpy as np
...
class YOLOv5Detector:
    ...
    """
    def __init__(self, yolo_model: str = 'yolov5s', conf_thres: float = 0.25, classes: List[int] = None, use_gt: bool = False):
        """
        Khởi tạo YOLOv5Detector
        Args:
            yolo_model: Tên mô hình YOLOv5 (yolov5s, yolov5m, v.v.)
        ...
        # Tải mô hình YOLOv5 (trừ khi chỉ sử dụng ground truth)
        if not use_gt:
            try:
                ...
                print(f"🚀 Đã tải mô hình YOLOv5 {yolo_model} trên {self.device}. Ngưỡng: {conf_thres}, Classes: {classes}")
